...
# HAS NOT BEEN TESTED YET 12/18/23
def calc_weight(start, end, edge, checkpoints, turns=None, turn_time=4, travel_time=1):
    ...
    for checkpoint in checkpoints:
        ...
        for coord1 in checkpoint:
            # DISTANCE FORMULAAAAAAAAAAAAAA
            for coord2 in path:
                distance = math.sqrt((coord1[0] - coord2[0])**2 + (coord1[1] - coord2[1])**2)
                if distance < min_distance:
                    min_distance = distance
                    nearest_checkpoint_coords.append(coord1)
    print(nearest_checkpoint_coords)
